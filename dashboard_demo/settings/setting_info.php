...
<ol class="breadcrumb">
 ...
 if(isset($_POST['password_save'])){
	 $getpassword = crypt(addslashes($_POST['old_password']),$userdata->password);
	 ...
	 if($userdata->password != $getpassword){
			...
			if(addslashes($_POST['new_password']) != addslashes($_POST['re_new_password'])){
				...
				if(addslashes($_POST['new_password']) != NULL && addslashes($_POST['re_new_password']) != NULL){
					 // A higher "cost" is more secure but consumes more processing power
						...
						$hash = crypt(addslashes($_POST['new_password']), $salt);
					$getdata->my_sql_update("user","password='".$hash."'","user_key='".$_SESSION['ukey']."'");
 ...
 <ul class="nav nav-tabs">
  ...
  <div class="panel panel-green">
                            ...
                            <div class="col-xs-9">
							 ...
							 while($showfont = mysql_fetch_object($getfont)){
								  if($showfont->font_key == $userdata->system_font_size){
									  echo '<div class="col-md-2"><button type="submit" name="change_font_size" class="btn btn-success btn-block" id="submit" value="'.$showfont->font_key.'">'.$showfont->font_name.'</button></div>';
								  }else{
                        ...
                        <div class="panel-footer">
                          <button type="submit" name="info_save" class="btn btn-success" disabled><i class="fa fa-save fa-fw"></i> <?php echo @LA_BTN_SAVE;?></button>
                        </div>
  </div>
</form>
                                </div>
...
<div class="panel panel-red">
                        ...
                        <div class="panel-body">
                           <div class="form-group">
                                               ...
                                               <input name="old_password" type="password" disabled="disabled"  class="form-control" id="old_password">
                           </div>
                           <div class="form-group">
                                               <label for="new_password"><?php echo @LA_LB_NEW_PASSWORD;?></label>
                                               <input name="new_password" type="password" disabled="disabled"  class="form-control" id="new_password">
                           </div>
                           <div class="form-group">
                                               <label for="re_new_password"><?php echo @LA_LB_NEW_PASSWORD_AGAIN;?></label>
                                               <input name="re_new_password" type="password" disabled="disabled"  class="form-control" id="re_new_password">
                           </div>
                        </div>
                        <div class="panel-footer" >
                            <button type="submit" name="password_save" class="btn btn-danger" disabled><i class="fa fa-unlock-alt fa-fw"></i> <?php echo @LA_LB_PASSWORD_CHANGE;?></button>
                        </div>
                    </div>
</form>
